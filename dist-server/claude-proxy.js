    ...
    return new Promise((resolve, reject) => {
        ...
        const finish = (err, result, usage) => {
            ...
            if (err)
                ...
                resolve({ result: result, usage: usage || null });
        };
        proc.stdout.on('data', (chunk) => {
            ...
            for (const line of lines) {
                ...
                if (type === 'assistant') {
                    ...
                    if (event['is_error']) {
                        ...
                        if (!accumulated)
                            finish(new Error(`Claude error: ${err}`), null, null);
                        else
                            finish(null, accumulated, null);
                    }
                    else {
        ...
        proc.on('close', (code) => {
            if (!settled) {
                if (accumulated)
                    ...
                    finish(new Error(`claude exited ${code}: ${stderr.slice(0, 200)}`), null, null);
            }
        });
        // 5 分钟超时
        setTimeout(() => {
