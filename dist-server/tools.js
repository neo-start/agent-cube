...
export const tools = {
    ...
    writeFile: {
        ...
        execute: async (args, workspace) => {
            ...
            if (firstNewline === -1)
                return 'Error: writeFile requires path on first line, content on subsequent lines';
    ...
    appendFile: {
        ...
        execute: async (args, workspace) => {
            const firstNewline = args.indexOf('\n');
            if (firstNewline === -1)
                return 'Error: appendFile requires path on first line, content after';
            const filePath = resolvePath(args.slice(0, firstNewline).trim(), workspace);
            const content = args.slice(firstNewline + 1);
            try {
                fs.mkdirSync(path.dirname(filePath), { recursive: true });
 ...
 */
export async function executeToolCalls(toolCalls, workspace) {
    const results = [];
