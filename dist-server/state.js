...
export function broadcast() {
    const payload = JSON.stringify(Object.fromEntries(Object.entries(state.agents).map(([name, a]) => [
        name,
...
export function pushGroupMsg(type, from, content, meta = {}) {
    ...
    if (!meta['partial']) {
        ...
        if (state.groupMessages[groupId].length > 500)
            state.groupMessages[groupId].shift();
