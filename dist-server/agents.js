...
import { loadMemory, appendMemory, logTask, loadScratchpad, loadSoul, appendLongTermMemory, saveThread } from './memory.js';
...
export function scheduleAgent(agentName, taskId, description) {
    ...
    if (agentState.status === 'working') {
        const enqueued = agentTaskQueues[agentName].enqueue(() => runAgent(agentName, taskId, description), { taskId, agent: agentName, description, createdAt: new Date().toISOString() });
        if (enqueued)
            persistQueues();
        if (!enqueued) {
            ...
            if (isGroupTask)
                pushGroupMsg('status', agentName, `Queue full (max 20), task dropped: ${description.slice(0, 60)}`, { taskId, groupId: task?.groupId || 'default' });
        }
        return;
    }
...
async function runAgent(agentName, taskId, description) {
    ...
    if (isGroupTask)
        pushGroupMsg('status', agentName, 'Thinking...', { status: 'working', taskId, groupId });
    ...
    try {
        ...
        if (agentConfig.provider === 'claude') {
            ...
            for (let iter = 0; iter < MAX_TOOL_ITERS; iter++) {
                const out = await provider.streamProvider(agentName, systemPrompt, currentPrompt, (_, accumulated) => {
                    ...
                    if (state.tasks[taskId])
                        state.tasks[taskId].latestLog = agent.latestLog;
                    if (streamMsg) {
                ...
                if (toolCalls.length === 0)
                    break;
                if (streamMsg)
                    pushGroupMsg('tool-call', agentName, `Executing ${toolCalls.length} tool(s): ${toolCalls.map(t => t.name).join(', ')}`, { taskId, groupId });
                const toolResults = await executeToolCalls(toolCalls, workspace);
                if (streamMsg)
                    pushGroupMsg('tool-result', agentName, toolResults.slice(0, 300), { taskId, groupId });
        ...
        if (!isDelegated && isGroupTask)
            checkGroupMessages(result, agentName, taskId);
        agent.status = 'done';
        if (!isDelegated)
            appendMemory(agentName, 'assistant', result.slice(0, 1000));
        if (state.tasks[taskId]) {
...
export function checkDelegation(response, fromAgent, taskId, originalDesc) {
    ...
    if (isGroupOrigin)
        pushGroupMsg('delegate', fromAgent, delegateDesc, { toAgent, taskId, groupId });
...
function parseNextDirective(text, thread, currentAgent) {
    ...
    if (thread && thread.participants.length >= 2) {
        ...
        if (other)
            return { type: 'next', targets: [other] };
    }
