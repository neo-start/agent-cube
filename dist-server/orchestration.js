...
async function watchBothTasks(orchestrationId, clawTaskId, deepTaskId, description, coderName, analystName) {
    ...
    const iv = setInterval(async () => {
        ...
        if (!clawTask || !deepTask)
            return;
        const clawDone = clawTask.status === 'done' || clawTask.status === 'blocked';
        const deepDone = deepTask.status === 'done' || deepTask.status === 'blocked';
        if (clawDone)
            state.orchestrations[orchestrationId]['clawResult'] = clawTask.result;
        if (deepDone)
            state.orchestrations[orchestrationId]['deepResult'] = deepTask.result;
...
export async function orchestrate(orchestrationId, description, groupId = 'default') {
    ...
    if (singleAgent) {
        ...
        try {
            ...
            if (parsed.route)
                routing = { ...parsed, route: parsed.route, reason: parsed.reason || '', clawTask: parsed.coderTask, deepTask: parsed.analystTask };
        }
        catch { }
    }
