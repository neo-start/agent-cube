...
export function loadTasksState() {
    try {
        if (!fs.existsSync(TASKS_STATE_FILE))
            return {};
...
export function loadQueuedTasks() {
    try {
        if (!fs.existsSync(QUEUED_TASKS_FILE))
            return {};
...
export function clearQueuedTasks() {
    try {
        fs.writeFileSync(QUEUED_TASKS_FILE, '{}');
    }
    catch { }
}
...
export function listThreads() {
    try {
        return fs.readdirSync(THREADS_DIR)
            ...
            .map(f => {
            try {
                return JSON.parse(fs.readFileSync(path.join(THREADS_DIR, f), 'utf-8'));
            }
            catch {
                return null;
            }
