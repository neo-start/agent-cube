import type { QueueTaskMeta } from './types.js';

interface QueueItem {
  fn: () => void;
  meta: QueueTaskMeta | null;
}
...
export class AgentTaskQueue {
  ...
  constructor(agentName: string, cap = 20) {
  ...
  enqueue(taskFn: () => void, meta: QueueTaskMeta | null): boolean {
  ...
  dequeue(): void {
    ...
    if (item?.fn) {
      try { item.fn(); } catch (err) {
        console.error(`[AgentTaskQueue:${this.agentName}] task error:`, (err as Error).message);
      }
    }
  }
